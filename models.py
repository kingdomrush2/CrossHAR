...
class LinearClassifier(nn.Module):
    def __init__(self, cfg, input=None, output=None):
        ...
        self.linear = nn.Linear(72*120, output)
...
class ConvNet(nn.Module):
    def __init__(self, input_shape, n_class: int, 
                 hidden_size = 128, kernel_sizes = [8, 5, 3]):
        '''
        input_shape (N, L): N is the number of channels and L is the time series length.
        '''
        super(ConvNet, self).__init__()

        self.instancenorm0 = nn.InstanceNorm1d(input_shape[0], affine=True)
    
        kernel_size = kernel_sizes[0]
        h1 = hidden_size
        self.padding1 = nn.ConstantPad1d((int((kernel_size-1)/2), int(kernel_size/2)), 0)
        self.conv1 = torch.nn.Conv1d(in_channels=input_shape[0], out_channels=h1, kernel_size=kernel_size)
        ...
        self.conv2 = torch.nn.Conv1d(in_channels=h1, out_channels=h2, kernel_size=kernel_size)
        ...
        self.conv3 = torch.nn.Conv1d(in_channels=h2, out_channels=h3, kernel_size=kernel_size)
        self.bn3 = nn.BatchNorm1d(num_features=h3)

        self.averagepool = nn.AvgPool1d(kernel_size=input_shape[1]//16)

        self.linear = nn.Linear(h3, n_class)

    def forward(self, input):
        input = input.transpose(1,2)
        x = self.padding1(input)
        x = self.conv1(x)
        #x = self.bn1(x)
        x = F.relu(x)
        x = F.max_pool1d(x, 4)
...
def fetch_classifier(method, model_cfg, input=None, output=None, feats=False):
    if 'lstm' in method:
        ...
        model = LinearClassifier(model_cfg, input=input, output=output)
    elif 'transformer' in method: 
        model = Transformer_ft(model_cfg, output_size=output)
    elif 'CNN' in method:
        model = ConvNet((72, 120), output, hidden_size=128, kernel_sizes=[5, 3, 3])
    else:
        model = None
    return model
